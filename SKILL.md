...
## When to Use
- **Deep Refactoring / Planning**: Use `--model gemini-3.6-pro --effort high` to delegate heavy multi-file edits or architectural reviews.
- **Fast Search / Isolated Execution**: Use `--model gemini-3.6-flash --effort low` for targeted search, unit test fixes, or script generation.
...
```bash
python /path/to/antigravity-subagent-launcher/agy_subagent.py --prompt "<COMPLEX_TASK_DESCRIPTION>" --model gemini-3.6-pro --effort high
```

### Plan Mode / Architecture Review
```bash
python /path/to/antigravity-subagent-launcher/agy_subagent.py --prompt "<RESEARCH_QUESTION>" --mode plan
```
...
| `--model` | `-m` | `gemini-3.6-flash` | Model choice: `gemini-3.6-pro`, `gemini-3.6-flash`. |
| `--effort` | `-e` | `high` | Reasoning effort level: `low`, `medium`, `high`. |
| `--workdir` | `-w` | `.` | Target project directory path (passed to agy via `--add-dir`). |
| `--mode` | | | Execution mode: `accept-edits` or `plan`. |
| `--no-skip-permissions` | | `false` | Disable auto-approval of tool permissions (by default `--dangerously-skip-permissions` is passed). |

## Notes & Environment Requirements
- Ensure `agy` CLI is accessible in PATH.
